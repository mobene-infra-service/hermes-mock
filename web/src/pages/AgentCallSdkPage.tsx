...
// 默认 SDK 脚本：0.0.14 裸地址（OSS bucket 公读时长期有效）。0.0.14 修复了 0.0.13 的拨号盘背景透明 bug。
// 注意：业务方有时给的是带 x-oss-signature 的临时签名 URL（约 9 小时过期），
// 那种地址过期会 403——手测时若 403，把 scriptUrl 换成最新签名 URL 即可（本字段可编辑）。
const DEFAULT_SCRIPT_URL = 'https://hermes-test-us.oss-us-west-1.aliyuncs.com/touch/sip-dialpad-sdk.umd.0.0.14.js'
...
export default function AgentCallSdkPage() {
  ...
  // lineType 先作为可选手填参数透传给 SDK；不由 mock 页面直接请求 Hermes line-types。
  // 等 SDK 暴露取线路类型方法后，再从 sdkRef.current 读取并填充选项。
  ...
  return (
    <div className="page-container">
      ...
      <InfoBanner title="官方 SIP Dialpad SDK · 直连 Hermes（不经 mock 反代）">
        本页用业务方提供的官方拨号盘 SDK（<Text code>window.SipDialpadSDK</Text>）复刻坐席外呼，并把 SDK 全部能力外露成表单：左侧填 <Text code>createSipDialpadSDK</Text> options 与 <Text code>RenderConfig</Text> 后「挂载 SDK」，右侧用 <Text code>makeCall</Text> 编程式外呼。线路类型 <Text code>lineType</Text> 当前仅作为可选参数手填透传，不由 mock 页面直接请求 Hermes；待 SDK 暴露取线路类型方法后再接入。拨号盘 UI 由 SDK 自渲染到下方容器。SDK 经 <Text code>serverAddr</Text> 直连 Hermes，与 <Text code>/agent-call</Text> 的 mock 自研软电话独立并存。需麦克风权限 + HTTPS/localhost；外部脚本需可访问 CDN。
      </InfoBanner>
        ...
        <Col xs={24} lg={12}>
          <Card size="small" title={<Space><ApiOutlined />挂载配置（createSipDialpadSDK + render）</Space>} style={{ marginBottom: 16 }}>
            ...
            >
              ...
              <Form.Item name="scriptUrl" label="SDK 脚本地址 scriptUrl" tooltip="默认 0.0.13 裸地址（OSS 公读长期有效）。业务方若给带 x-oss-signature 的临时签名 URL（约 9 小时过期），过期会 403，换最新签名 URL 即可" rules={[{ required: true, message: '必填' }]}>
              ...
              <Row gutter={12}>
                ...
                <Col span={12}>
                  <Form.Item
                    name="lineType"
                    label="线路类型 lineType"
                    tooltip="可选手填参数，随 makeCall 传给 SDK；mock 页面不直接请求 /line-types，后续等 SDK 提供方法后再自动填充"
                  >
                    <Input allowClear placeholder="可选，如 base；留空则不传" />
                  </Form.Item>
                </Col>
                <Col span={12}>
