...
> **接口契约（字段/示例/参数）的权威源** = hermes 仓 `~/IdeaProjects/hermes/docs/stratflow-mock-openapi-spec.md`。
...
- Go client（`internal/hermesopenapi`，复用共用网关/凭据）：
  gate 组、`config` 组、`plans`、`all`；发现组 `workflows / workflows/{defCode} / collections[?name&status] / collections/{code}/fields / collections/{code}/bindings / collections/{code}/runs / runs/{rid}/progress[?uploadStartTime&uploadEndTime]`；触发 `import`（解析 `plans[].{result,runCode,versionCode,failFields}`）。
