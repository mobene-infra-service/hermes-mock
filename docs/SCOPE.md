...
开发者 ──配 mock 线路 address──▶ Hermes (basic: t_line.address = mock)
        Hermes 业务层发起外呼 (call-center 群呼 / call-bot / OTP / 坐席外呼) ── 选 mock 线路
                         ...
                         按【客户行为档】决定：接听 / 拒接 / 振铃不接 / 放音 / DTMF / 挂断 / 故障
                                                           │
                采集真实 SIP（INVITE/响应码/BYE + 原始报文）→ 落 mock_call_record → 断言通过/失败
```
...
- **批量客户**：客户组（`mock_customer_group`）= 一个号段 N 个虚拟客户，引用行为档、绑定线路；改组状态/行为档 → 整批生效。
- **个例覆盖**（`mock_customer_override`）：组内个别号码的例外行为/状态。
- **线路绑定**（`mock_line_binding`）：客户组 ↔ Hermes 线路（`t_line.address→mock`）；多线路绑不同客户组测换线。
- **真实 SIP 采集**：传输层抓原始报文，按 Call-ID/callUuid 聚合成链路，落库供断言与排障。
