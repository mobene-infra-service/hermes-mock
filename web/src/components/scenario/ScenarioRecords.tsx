...
// scenario 用 LIKE 过滤（如 'callcenter-task' / 'otp' / 'agent' 命中 agent-call+agent-inbound）。
// 两视图：通话记录(call-records，按 scenario) + 测试历史(test runs，按 caseKinds)。默认开自动刷新——
// 预测式拨号/转接进展会随时间变化，自动刷新让卡片不再停在「那一刻」。
export interface ScenarioRecordsProps {
  scenario: string // 主场景记录的 LIKE 过滤值
...
export default function ScenarioRecords({ scenario, caseKinds, title = '本场景记录', altLabel, altScenario }: ScenarioRecordsProps) {
  ...
  const load = useCallback(async (silent = false) => {
    ...
    try {
      if (view === 'runs') {
        ...
        const r = await queryCallRecords({ scenario: activeScenario, page: pageRef.current, pageSize: PAGE_SIZE })
  ...
  }, [view, activeScenario, caseKinds])

  useEffect(() => { void load() }, [load])
  // 自动刷新（静默，不闪 loading）。组件不可见时（如常驻但 display:none 的坐席软电话内）跳过，避免后台空轮询。
  useEffect(() => {
    if (!auto) return
    const t = setInterval(() => {
    ...
    >
      {view === 'runs' ? (
        ...
        <Table<CallRecord>
          ...
          pagination={{ current: page, pageSize: PAGE_SIZE, total, hideOnSinglePage: true, size: 'small', showSizeChanger: false }}
    ...
    <Space direction="vertical" size="middle" style={{ width: '100%' }}>
      <Descriptions size="small" column={1} bordered
        items={[
          ...
          ...(r.sipCallId ? [{ key: 'sip', label: 'SIP Call-ID', children: <Text style={{ fontSize: 11 }}>{r.sipCallId}</Text> }] : []),
        ]}
      />
      <Space wrap>
        {r.signalSummary && <Tag color="blue">信令 {r.signalSummary}</Tag>}
        {r.mediaSummary && <Tag color="green">媒体 {r.mediaSummary}</Tag>}
        {r.callbackSummary && <Tag color="purple">回调 {r.callbackSummary}</Tag>}
      </Space>
      {steps.length > 0 && (
        <div>
